...
class Dependencies(globalModuleFilter: ModuleID => ModuleID = (m:ModuleID)=>m) {
  ...
  def slf4s(v: String = "latest.release") = "com.weiglewilczek.slf4s" %% "slf4s" % v applyGlobal() //"slf4s_2.9.1"
  ...
  def subcut(v: String = "latest.release") = "org.scala-tools.subcut" %% "subcut" % v applyGlobal()
